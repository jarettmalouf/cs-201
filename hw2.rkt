...
(define (prod tree)
  (cond
    ...
    [(number? tree) tree]
    [(not (list? tree)) 'none]
    [else (let ((frst [prod (car tree)]) (scnd [prod (cdr tree)]))
            (cond [(and (number? frst) (number? scnd)) (* frst scnd)]
                  [(number? frst) frst]
                  [else scnd]))]))
...
(define (count-if pred tree)
  (cond
    ...
    [(and (not (list? tree)) (not (pred tree)) 0)]
...
(define (average tree)
  (let ((sm [sum tree]) (ct [count-if number? tree]))
  (cond
    [(= ct 0) 'NA]
    [else (/ sm ct)])))
...
(define (tree-replace old new tree)
  (cond
    [(empty? tree) empty]
    [(equal? old tree) new]
    [(list? tree) (cons (tree-replace old new (car tree)) (tree-replace old new (cdr tree)))]
    [else tree]))
...
(define (tree-min tree)
  (cond
    [(empty? tree) '()]
    [(and (list?  tree) (not (equal? (car tree) null)) (not (equal? (cdr tree) null)))
     (min (tree-min (car tree)) (tree-min (cdr tree)))]
    [(and (list?  tree) (equal? (car tree) null)) (tree-min (cdr tree))]
    [(and (list?  tree) (equal? (cdr tree) null)) (tree-min (car tree))]
    [(integer? tree) tree]))
